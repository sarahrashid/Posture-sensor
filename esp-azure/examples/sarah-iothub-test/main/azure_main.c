...
// Sets GPIO Pins for LED and Buzzer
#define GPIO_OUTPUT_IO_LED 0
#define GPIO_OUTPUT_IO_BUZZER 5
#define GPIO_OUTPUT_PIN_SEL (1ULL << GPIO_OUTPUT_IO_BUZZER) | (1ULL << GPIO_OUTPUT_IO_LED)   // Bit mask
...
// Set the delay message and the duration the device is buzzing/vibrating
const TickType_t MESSAGE_DELAY_SECONDS = 1;
const int BUZZER_DURATION_SECONDS = 3;

// For the mosfet. When setting GPIO power consumption to turn on or off is high or low (here up or down)
const int GPIO_PULL_DOWN = 0;
const int GPIO_PULL_UP = 1;

// Creating handle of the low level IoT hub device client
IOTHUB_DEVICE_CLIENT_LL_HANDLE hubDeviceClient;

static const char *TAG = "azure";
static PostureSensor *postureSensorCurrent;

// Initialize the posture sensor
static void initPostureSensor()
{
    postureSensorCurrent = malloc(sizeof(PostureSensor));  // allocating memory based on size of items in struct
...
// for esp to initialize the wifi
...
// Method sets the Buzzer to high for the duration given
...
// The azure task where everything happens! FreeRTOS works in tasks
    ...
    while (1)
    {
        if (ESP_OK == adc_read(&strain_level))
        {
            ...
            if (result == 0)
            {
                ...
                gpio_set_level(GPIO_OUTPUT_IO_LED, 1); // red led blinks when message sent
            }
            else
            {
        ...
        //trace statements here for debugging
